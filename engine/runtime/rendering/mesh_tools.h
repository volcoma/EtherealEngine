...
{
	// all of these produce shapes with center in (0, 0, 0) and each vertex
	// magnitude of 1 (normalized),
	// except where specified otherwise
	// front faces are counter-clockwise
	static void create_plane(std::vector<math::vec3>& out_vertices, std::vector<std::uint32_t>& out_indices,
							 float size_x, float size_y, bool y_up = true);
	static void create_tetrahedron(std::vector<math::vec3>& out_vertices,
								   std::vector<std::uint32_t>& out_indices, bool share_vertices,
								   bool y_up = true);
	static void create_cube(std::vector<math::vec3>& out_vertices, std::vector<std::uint32_t>& out_indices,
							bool share_vertices, float edge_half_length = 0.7071067811865475f,
							bool y_up = true);
	static void create_octahedron(std::vector<math::vec3>& out_vertices,
								  std::vector<std::uint32_t>& out_indices, bool share_vertices,
								  bool y_up = true);
	static void create_icosahedron(std::vector<math::vec3>& out_vertices,
								   std::vector<std::uint32_t>& out_indices, bool share_vertices,
								   bool y_up = true);
	static void create_dodecahedron(std::vector<math::vec3>& out_vertices,
									std::vector<std::uint32_t>& out_indices, bool share_vertices,
									bool y_up = true);
	static void create_icosphere(std::vector<math::vec3>& out_vertices,
								 std::vector<std::uint32_t>& out_indices, int tessellation_level,
								 bool share_vertices, bool y_up = true);
	static void create_cylinder(std::vector<math::vec3>& out_vertices,
								std::vector<std::uint32_t>& out_indices, float height, float radius_bottom,
								float radius_top, int tessellation, bool open_top_bottom, bool share_vertices,
								bool y_up = true);
	static void create_teapot(std::vector<math::vec3>& out_vertices, std::vector<std::uint32_t>& out_indices,
							  bool y_up = true);
	...
	template <typename VertexType>
	static inline int find_or_add(std::vector<VertexType>& vertices, const VertexType& vert,
								  int searchForDuplicates)
	{
		for(int i = (int)(vertices.size()) - 1; (i >= 0) && (i >= (int)vertices.size() - searchForDuplicates);
			i--)
		{
			if(vertices[i] == vert)
				return i;
	...
	template <typename VertexType>
	static inline void add_triangle(std::vector<VertexType>& out_vertices,
									std::vector<std::uint32_t>& out_indices, const VertexType& v0,
	...
	template <typename VertexType>
	static inline void add_triangle_merge_same_position_vertices(
		std::vector<VertexType>& out_vertices, std::vector<std::uint32_t>& out_indices, const VertexType& v0,
		const VertexType& v1, const VertexType& v2, int vertex_merging_look_from_vertex_offset = 0)
	{
		int i0 =
			find_or_add(out_vertices, v0, (int)out_vertices.size() - vertex_merging_look_from_vertex_offset);
		int i1 =
			find_or_add(out_vertices, v1, (int)out_vertices.size() - vertex_merging_look_from_vertex_offset);
		int i2 =
			find_or_add(out_vertices, v2, (int)out_vertices.size() - vertex_merging_look_from_vertex_offset);

		add_triangle(out_indices, i0, i1, i2);
	}

	// This adds quad triangles in strip order ( (0, 0), (1, 0), (0, 1), (1, 1) )
	// - so swap the last two if
	// doing clockwise/counterclockwise
	...
	// (this is a bit inconsistent with add_pentagon below)
	template <typename VertexType>
	static inline void add_quad(std::vector<VertexType>& out_vertices,
								std::vector<std::uint32_t>& out_indices, const VertexType& v0,
								const VertexType& v1, const VertexType& v2, const VertexType& v3)
	{
		int i0 = add_vertex(out_vertices, v0);
		int i1 = add_vertex(out_vertices, v1);
		int i2 = add_vertex(out_vertices, v2);
	...
	static inline void add_pentagon(std::vector<std::uint32_t>& out_indices, int i0, int i1, int i2, int i3,
									int i4)
	{
		add_triangle(out_indices, i0, i1, i2);
	...
	// (this is a bit inconsistent with add_quad above)
	template <typename VertexType>
	static inline void add_pentagon(std::vector<VertexType>& out_vertices,
									std::vector<std::uint32_t>& out_indices, const VertexType& v0,
									const VertexType& v1, const VertexType& v2, const VertexType& v3,
									const VertexType& v4)
	{
	...
	static inline void generate_normals(std::vector<math::vec4>& out_normals,
										const std::vector<math::vec3>& vertices,
										const std::vector<std::uint32_t>& indices, bool counter_clockwise,
										int index_from = 0, int index_count = -1,
										bool fix_broken_normals = true)
	{
	...
	static void
	generate_tangents(std::vector<math::vec4>& out_tangents, std::vector<math::vec4>& out_bitangents,
					  const std::vector<math::vec3>& vertices, const std::vector<math::vec4>& normals,
					  const std::vector<math::vec2>& texturecoords, const std::vector<std::uint32_t>& indices)
	{
	...
	// dummy tangents, for better, http://www.terathon.com/code/tangent.html or
	// http://developer.nvidia.com/object/NVMeshMender.html
	static void fill_dummy_ttt(const std::vector<math::vec3>& vertices,
							   const std::vector<math::vec4>& normals, std::vector<math::vec4>& tangents,
							   std::vector<math::vec2>& texcoords0, std::vector<math::vec2>& texcoords1)
	{
