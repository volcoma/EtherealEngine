...
static DebugShapeVertex s_cubeVertices[8] = {
	{-1.0f, 1.0f, 1.0f, {0, 0, 0, 0}},   {1.0f, 1.0f, 1.0f, {0, 0, 0, 0}},
	{-1.0f, -1.0f, 1.0f, {0, 0, 0, 0}},  {1.0f, -1.0f, 1.0f, {0, 0, 0, 0}},
	{-1.0f, 1.0f, -1.0f, {0, 0, 0, 0}},  {1.0f, 1.0f, -1.0f, {0, 0, 0, 0}},
	{-1.0f, -1.0f, -1.0f, {0, 0, 0, 0}}, {1.0f, -1.0f, -1.0f, {0, 0, 0, 0}},
};
		...
		struct Gen
		{
				...
				, m_normalStride(_normalStride)
			{
				...
				static const float vv[12][4] = {
					{-ll, 0.0f, -ss, 0.0f}, {ll, 0.0f, -ss, 0.0f},
					{ll, 0.0f, ss, 0.0f},   {-ll, 0.0f, ss, 0.0f},

					{-ss, ll, 0.0f, 0.0f},  {ss, ll, 0.0f, 0.0f},
					{ss, -ll, 0.0f, 0.0f},  {-ss, -ll, 0.0f, 0.0f},

					{0.0f, -ss, ll, 0.0f},  {0.0f, ss, ll, 0.0f},
					{0.0f, ss, -ll, 0.0f},  {0.0f, -ss, -ll, 0.0f},
				};
...
struct DebugDraw
{
	...
	void init(bool _depthTestLess, bx::AllocatorI* _allocator)
	{
		...
		for(uint32_t mesh = 0; mesh < 4; ++mesh)
		{
			...
			uint32_t numLineListIndices = bgfx::topologyConvert(bgfx::TopologyConvert::TriListToLineList,
																NULL, 0, trilist, numIndices, false);
			indices[id] =
				(uint16_t*)BX_ALLOC(m_allocator, (numIndices + numLineListIndices) * sizeof(uint16_t));
		...
		for(uint32_t mesh = Mesh::Sphere0; mesh < Mesh::Cube; ++mesh)
		{
			...
			bx::memCopy(&vb->data[m_mesh[id].m_startVertex * stride], vertices[id],
		...
		bx::memCopy(&vb->data[m_mesh[Mesh::Cube].m_startVertex * stride], s_cubeVertices,
					sizeof(s_cubeVertices));

		bx::memCopy(&ib->data[m_mesh[Mesh::Cube].m_startIndex[0] * sizeof(uint16_t)], s_cubeIndices,
	...
	void begin(uint8_t _viewId)
	{
		...
		attrib.m_state = 0 | BGFX_STATE_RGB_WRITE |
						 (m_depthTestLess ? BGFX_STATE_DEPTH_TEST_LESS : BGFX_STATE_DEPTH_TEST_GREATER) |
						 BGFX_STATE_CULL_CW | BGFX_STATE_DEPTH_WRITE;
		attrib.m_scale = 1.0f;
		attrib.m_spin = 0.0f;
	...
	void setState(bool _depthTest, bool _depthWrite, bool _clockwise)
	{
		...
		uint64_t state = m_attrib[m_stack].m_state &
						 ~(0 | BGFX_STATE_DEPTH_TEST_MASK | BGFX_STATE_DEPTH_WRITE | BGFX_STATE_CULL_CW |
						   BGFX_STATE_CULL_CCW);

		state |= _depthTest ? depthTest : 0;

		state |= _depthWrite ? BGFX_STATE_DEPTH_WRITE : 0;
	...
	void draw(const Sphere& _sphere)
	{
		const Attrib& attrib = m_attrib[m_stack];
		float mtx[16];
		bx::mtxSRT(mtx, _sphere.m_radius, _sphere.m_radius, _sphere.m_radius, 0.0f, 0.0f, 0.0f,
				   _sphere.m_center[0], _sphere.m_center[1], _sphere.m_center[2]);
		uint8_t lod = attrib.m_lod > Mesh::SphereMaxLod ? uint8_t(Mesh::SphereMaxLod) : attrib.m_lod;
	...
	void drawArc(Axis::Enum _axis, float _x, float _y, float _z, float _radius, float _degrees)
	{
		...
		float pos[3];
		getPoint(pos, _axis, bx::fsin(step * 0) * _radius, bx::fcos(step * 0) * _radius);
		...
		{
			getPoint(pos, _axis, bx::fsin(step * ii) * _radius, bx::fcos(step * ii) * _radius);
		...
		getPoint(pos, _axis, bx::fsin(step * n) * _radius, bx::fcos(step * n) * _radius);
		moveTo(pos[0] + _x, pos[1] + _y, pos[2] + _z);
		lineTo(_x, _y, _z);
	}
	...
	void drawCircle(Axis::Enum _axis, float _x, float _y, float _z, float _radius, float _weight)
	{
		...
		float pos[3];
		getPoint(pos, _axis, bx::flerp(xy0[0], xy1[0], _weight) * _radius,
	...
	void drawCone(const float* _from, const float* _to, float _radius)
	{
		...
		uint8_t lod = attrib.m_lod > Mesh::ConeMaxLod ? uint8_t(Mesh::ConeMaxLod) : attrib.m_lod;
	...
	void drawCylinder(const float* _from, const float* _to, float _radius, bool _capsule)
	{
		...
		if(_capsule)
		{
			uint8_t lod = attrib.m_lod > Mesh::CapsuleMaxLod ? uint8_t(Mesh::CapsuleMaxLod) : attrib.m_lod;
		...
		{
			uint8_t lod = attrib.m_lod > Mesh::CylinderMaxLod ? uint8_t(Mesh::CylinderMaxLod) : attrib.m_lod;
...
private:
	...
	void draw(Mesh::Enum _mesh, const float* _mtx, uint16_t _num, bool _wireframe) const
	{
		...
		{
			bgfx::setIndexBuffer(m_ibh, mesh.m_startIndex[_wireframe], mesh.m_numIndices[_wireframe]);
		}
		...
		float params[4][4] = {
			...
			{
				// matColor
				((attrib.m_abgr) & 0xff) / 255.0f, ((attrib.m_abgr >> 8) & 0xff) / 255.0f,
				((attrib.m_abgr >> 16) & 0xff) / 255.0f, (alpha) / 255.0f,
			},
		};
		...
		bgfx::setState(0 | attrib.m_state |
	...
	void flush()
	{
		if(0 != m_pos)
		{
			if(checkAvailTransientBuffers(m_pos, DebugVertex::ms_decl, m_indexPos))
			{
				...
				bgfx::setState(0 | BGFX_STATE_RGB_WRITE | BGFX_STATE_PT_LINES | attrib.m_state |
	...
	void flushQuad()
	{
		if(0 != m_posQuad)
		{
			...
			if(checkAvailTransientBuffers(m_posQuad, DebugUvVertex::ms_decl, numIndices))
			{
				...
				bgfx::setVertexBuffer(0, &tvb);
				bgfx::setIndexBuffer(&tib);
				bgfx::setState(0 | (attrib.m_state & ~BGFX_STATE_CULL_MASK));
				bgfx::setTransform(m_mtx);
