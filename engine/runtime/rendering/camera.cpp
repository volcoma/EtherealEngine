...
const math::frustum & camera::get_frustum()
{
	...
	if (_frustum_dirty == true && _frustum_locked == false)
	{
		...
		_clipping_volume.planes[math::volume_plane::far_plane].data.w = -_clipping_volume.planes[math::volume_plane::near_plane].data.w; // At near plane
		_clipping_volume.planes[math::volume_plane::near_plane].data.w = -math::dot((math::vec3&)_clipping_volume.planes[math::volume_plane::near_plane], get_position()); // At camera

		// The corner points also need adjusting in this case such that they sit
		// precisely on the new planes.
		_clipping_volume.points[math::volume_geometry_point::left_bottom_far] = _clipping_volume.points[math::volume_geometry_point::left_bottom_near];
		_clipping_volume.points[math::volume_geometry_point::left_top_far] = _clipping_volume.points[math::volume_geometry_point::left_top_near];
		_clipping_volume.points[math::volume_geometry_point::right_bottom_far] = _clipping_volume.points[math::volume_geometry_point::right_bottom_near];
		_clipping_volume.points[math::volume_geometry_point::right_top_far] = _clipping_volume.points[math::volume_geometry_point::right_top_near];
...
math::vec3 camera::estimate_pick_tolerance(float WireTolerance, const math::vec3 & Pos, const math::transform & ObjectTransform)
{
