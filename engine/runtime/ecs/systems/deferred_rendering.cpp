...
{
    ecs.for_each<camera_component>(
        [this, &ecs, dt](entity ce, camera_component& camera_comp)
        {
            auto& camera_lods = lod_data_[ce];
                                                                     ...
                                                                     delta_t dt)
{
    ...
    ecs.for_each<transform_component, light_component>(
        [this, &camera, &pass, &buffer_size, &view, &proj, g_buffer_fbo, refl_buffer](
            entity e,
                                                                             ...
                                                                             delta_t dt)
{
    ...
    pass.clear(BGFX_CLEAR_COLOR, 0, 0.0f, 0);
    ecs.for_each<transform_component, reflection_probe_component>(
        [this, &camera, &pass, &buffer_size, &view, &proj, g_buffer_fbo](entity e,
                                                                         transform_component& transform_comp_ref,
                                                                         ...
                                                                         delta_t dt)
{
    ...
    if((surface != nullptr) && atmospherics_program_)
    {
        ...
        ecs.for_each<transform_component, light_component>(
            [&light_direction,
             &found_sun](entity e, transform_component& transform_comp_ref, light_component& light_comp_ref)
            {
                if(found_sun)
                {
                    return;
...
std::shared_ptr<gfx::frame_buffer> deferred_rendering::tonemapping_pass(std::shared_ptr<gfx::frame_buffer> input,
                                                                        camera& camera,
                                                                        gfx::render_view& render_view)
{
    if(!input)
...
deferred_rendering::deferred_rendering()
{
    ...
    ts.push_or_execute_on_owner_thread(
        [this](asset_handle<gfx::shader> vs, asset_handle<gfx::shader> fs)
        {
            spot_light_program_ = std::make_unique<gpu_program>(vs, fs);
        },
        vs_clip_quad,
    ...
    ts.push_or_execute_on_owner_thread(
        [this](asset_handle<gfx::shader> vs, asset_handle<gfx::shader> fs)
        {
            gamma_correction_program_ = std::make_unique<gpu_program>(vs, fs);
        },
        vs_clip_quad,
        fs_gamma_correction);
    ...
    ts.push_or_execute_on_owner_thread(
        [this](asset_handle<gfx::shader> vs, asset_handle<gfx::shader> fs)
        {
            sphere_ref_probe_program_ = std::make_unique<gpu_program>(vs, fs);
        },
        vs_clip_quad_ex,
    ...
    ts.push_or_execute_on_owner_thread(
        [this](asset_handle<gfx::shader> vs, asset_handle<gfx::shader> fs)
        {
            box_ref_probe_program_ = std::make_unique<gpu_program>(vs, fs);
        },
        vs_clip_quad_ex,
    ...
    ts.push_or_execute_on_owner_thread(
        [this](asset_handle<gfx::shader> vs, asset_handle<gfx::shader> fs)
        {
            atmospherics_program_ = std::make_unique<gpu_program>(vs, fs);
        },
        vs_clip_quad_ex,
        fs_atmospherics);
}
