...
namespace math
{
	...
	void bbox::get_plane_points(VolumePlane::Side side, vec3 points_out[]) const
	{
		...
		case VolumePlane::Near:
			...
			points_out[3].x = max.x; points_out[3].y = min.y; points_out[3].z = min.z;
			break;
	...
	/// </summary>
	//-----------------------------------------------------------------------------
	bbox& bbox::from_points(const char * point_buffer, unsigned int point_count, unsigned int point_stride, bool reset_bounds /* = true */)
	{
	...
	bool bbox::intersect(const bbox& bounds, bool & contained) const
	{
		...
		if (bounds.min.x < min.x || bounds.min.x > max.x) contained = false;
		else if (bounds.min.y < min.y || bounds.min.y > max.y) contained = false;
		else if (bounds.min.z < min.z || bounds.min.z > max.z) contained = false;
		else if (bounds.max.x < min.x || bounds.max.x > max.x) contained = false;
		else if (bounds.max.y < min.y || bounds.max.y > max.y) contained = false;
		else if (bounds.max.z < min.z || bounds.max.z > max.z) contained = false;
	...
	bool bbox::intersect(const vec3 & v_tri0, const vec3 & v_tri1, const vec3 & v_tri2, const bbox & tri_bounds) const
	{
		...
		if (tri_bounds.min.x > max.x || tri_bounds.max.x < min.x ||
			tri_bounds.min.y > max.y || tri_bounds.max.y < min.y ||
