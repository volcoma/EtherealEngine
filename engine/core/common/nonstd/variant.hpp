#ifndef NONSTD_UTIL_VARIANT_HPP
#define NONSTD_UTIL_VARIANT_HPP
...
# ifdef __GNUC__
#  define NONSTD_VARIANT_DEPRECATED __attribute__((deprecated))
# elif defined(_MSC_VER)
#  define NONSTD_VARIANT_DEPRECATED __declspec(deprecated)
# else
#  define NONSTD_VARIANT_DEPRECATED
# endif
#else
#  define NONSTD_VARIANT_DEPRECATED [[deprecated]]
#endif
...
#if !defined(NONSTD_VARIANT_MINIMIZE_SIZE)
using type_index_t = unsigned int;
#else
#if defined(NONSTD_VARIANT_OPTIMIZE_FOR_SPEED)
...
struct direct_type<T, First, Types...>
{
	static constexpr type_index_t index =
		std::is_same<T, First>::value ? sizeof...(Types) : direct_type<T, Types...>::index;
};
...
{
	static auto apply_const(recursive_wrapper<T> const& obj) -> typename recursive_wrapper<T>::type const&
	{
		return obj.get();
	}
...
struct unwrapper<std::reference_wrapper<T>>
{
	...
	static auto apply(std::reference_wrapper<T>& obj) -> typename std::reference_wrapper<T>::type&
	{
		return obj.get();
	}
};
...
class variant
{
	...
	static_assert(sizeof...(Types) < std::numeric_limits<type_index_t>::max(),
...
public:
	...
	template <
		...
		typename Enable = typename std::enable_if<
			Traits::is_valid && !std::is_same<variant<Types...>, typename Traits::value_type>::value>::type>
	VARIANT_INLINE
	variant(T&& val) noexcept(std::is_nothrow_constructible<typename Traits::target_type, T&&>::value)
...
public:
	...
	template <typename T,
			  typename std::enable_if<(detail::direct_type<recursive_wrapper<T>, Types...>::index !=
									   detail::invalid_value)>::type* = nullptr>
	VARIANT_INLINE bool is() const
	{
	...
	template <typename T,
			  typename std::enable_if<(detail::direct_type<std::reference_wrapper<T>, Types...>::index !=
									   detail::invalid_value)>::type* = nullptr>
	VARIANT_INLINE T& get_unchecked()
	{
	...
	template <
		typename T,
		typename std::enable_if<(detail::direct_type<std::reference_wrapper<T const>, Types...>::index !=
								 detail::invalid_value)>::type* = nullptr>
	VARIANT_INLINE T const& get_unchecked() const
	{
	...
	// Use which() instead.
	NONSTD_VARIANT_DEPRECATED VARIANT_INLINE type_index_t get_type_index() const
	{
		return type_index;
	}
	...
	auto VARIANT_INLINE match(Fs&&... fs) const
		-> decltype(variant::visit(*this, ::nonstd::util::make_visitor(std::forward<Fs>(fs)...)))
	{
		return variant::visit(*this, ::nonstd::util::make_visitor(std::forward<Fs>(fs)...));
	}
	// non-const
	template <typename... Fs>
