...
typename detail::dynamic_caster<T>::result_type dynamic_variant_cast(V& v)
{
	return nonstd::apply_visitor(detail::dynamic_caster<T>(), v);
}

template <class T, class V>
typename detail::dynamic_caster<const T>::result_type dynamic_variant_cast(const V& v)
{
