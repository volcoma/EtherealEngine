...
//#define HPP_INPLACE_FUNCTION_USE_STD_ALIGNED_STORAGE
#ifndef HPP_INPLACE_FUNCTION_USE_STD_ALIGNED_STORAGE
...
template <class F, class... Args>
struct is_invocable_r_impl<decltype(std::declval<F>()(std::declval<Args>()...), void()), const void, F,
                           Args...> : std::true_type
{
};
...
private:
    ...
    constexpr static void check_size()
    {
        static_assert(RealSize <= Capacity, "inplace_function cannot be constructed from object with this (large) size");
    }
    template <typename ToCheck, std::size_t RealAlignment = alignof(ToCheck)>
    constexpr static void check_alignment()
    {
        static_assert(Alignment % RealAlignment == 0, "inplace_function cannot be constructed from object with this (large) alignment");
    }
};
