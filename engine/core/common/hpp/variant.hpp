...
struct base
{
	...
	inline static constexpr int visit_visitor_return_type_check()
	{
		static_assert(lib::all<std::is_same<F, Fs>::value...>::value,
					  "`visit` requires the visitor to have a single "
					  "return type.");
		return 0;
	}
...
template <typename... Ts>
struct hash<STX_NAMESPACE_NAME::detail::enabled_type<
	STX_NAMESPACE_NAME::variant<Ts...>, STX_NAMESPACE_NAME::lib::enable_if_t<STX_NAMESPACE_NAME::lib::all<
							   STX_NAMESPACE_NAME::detail::hash::is_enabled<STX_NAMESPACE_NAME::lib::remove_const_t<Ts>>()...>::value>>>
{
	using argument_type = STX_NAMESPACE_NAME::variant<Ts...>;
	using result_type = std::size_t;

	inline result_type operator()(const argument_type& v) const
	{
		using STX_NAMESPACE_NAME::detail::visitation::variant;
		std::size_t result = v.valueless_by_exception()
								 ? 299792458 // Random value chosen by the universe upon creation
								 : variant::visit_alt(
#ifdef MPARK_GENERIC_LAMBDAS
									   [](const auto& alt) {
										   using alt_type = STX_NAMESPACE_NAME::lib::decay_t<decltype(alt)>;
										   using value_type =
											   STX_NAMESPACE_NAME::lib::remove_const_t<typename alt_type::value_type>;
...
private:
	...
	struct hasher
	{
		...
		{
			using alt_type = STX_NAMESPACE_NAME::lib::decay_t<Alt>;
			using value_type = STX_NAMESPACE_NAME::lib::remove_const_t<typename alt_type::value_type>;
			return hash<value_type>{}(alt.value);
		}
	};
#endif
...
{
	using argument_type = STX_NAMESPACE_NAME::monostate;
	using result_type = std::size_t;
