...
#include "../common/hpp/type_index.hpp"
#include <functional>
#include <memory>
#include <set>
#include <sstream>
...
private:
    ...
    {
        static inline std::string get(const std::vector<std::string>& argumentNames,
                               ...
                               const std::function<void(Args...)>& callback)
{
    ...
    cmd->call = [this, commandRaw, callback, argCount](std::vector<std::string>& arguments)
    {
        ...
        else
        {
            ...
            arguments.insert(arguments.end(),
                             commandRaw->default_arguments.begin() + offset,
    ...
    cmd->get_usage = [this, commandRaw]()
    {
        ...
        return "Usage: " + commandRaw->name +
               NameArguments<Args...>::get(commandRaw->argument_names, 0, static_cast<unsigned int>(requiredArguments));
    };
