...
*
* @tparam     S     The max size of available handles, should be less than
* handle::invalid.
*/
template <size_t N>
struct handle_set
{
	static_assert(N < handle::invalid, "The max size of handle set should be less than Handle::invalid.");
...
/**
* @brief      Dynamic version of handle_set, the capacity will increase
* automaticly.
*
*/
...
template <size_t N>
inline typename std::size_t handle_set<N>::size() const
{
	return N - _available;
}
...
inline typename handle_set<N>::const_iterator_t handle_set<N>::begin() const
{
	...
	return const_iterator_t(*this, (_versions[0] & 0x1) == 1 ? handle : find_next_available(handle));
}
...
template <size_t N>
inline handle handle_set<N>::find_next_available(handle hndl) const
{
	for(index_t i = (hndl.get_index() + 1); i < static_cast<index_t>(N - _available); ++i)
	{
