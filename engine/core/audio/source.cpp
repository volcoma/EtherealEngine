...
void source::set_playing_offset(sound_info::seconds_t offset)
{
	if(is_valid())
	{
...
sound_info::seconds_t source::get_playing_offset() const
{
	if(is_valid())
	{
		return sound_info::seconds_t(impl_->get_playing_offset());
	}
	return sound_info::seconds_t(0);
}

sound_info::seconds_t source::get_playing_duration() const
{
	if(is_valid())
	{
		return sound_info::seconds_t(impl_->get_playing_duration());
	}
	return sound_info::seconds_t(0);
}
