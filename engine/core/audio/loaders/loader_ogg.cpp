...
bool load_ogg_from_memory(const uint8_t* data, std::size_t data_size, sound_data& result, std::string& err)
{
	...
	std::size_t data_bytes = std::size_t(stb_vorbis_stream_length_in_samples(oss) * std::size_t(info.channels)) *
	...
	stb_vorbis_get_samples_short_interleaved(oss, info.channels,
											 reinterpret_cast<std::int16_t*>(result.data.data()), int(data_bytes));
