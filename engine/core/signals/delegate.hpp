...
public:
	...
	delegate(delegate&& d)
		: object_ptr_(d.object_ptr_), stub_ptr_(d.stub_ptr_), deleter_(d.deleter_), store_(d.store_),
	...
	{
		// comparison between functor and non-functor is left as undefined at the
		// moment.
		if(store_size_ && rhs.store_size_) // both functors
			return (std::memcmp(store_.get(), rhs.store_.get(), store_size_) == 0) &&
