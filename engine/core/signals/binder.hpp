...
class function_wrapper<F, nonstd::index_sequence<ArgCount...>> : public base_function_wrapper
{
	...
	nonstd::any invoke_variadic_impl(nonstd::index_sequence<Arg_Idx...>, const std::vector<nonstd::any>& arg_list) const
	{
		...
		//assert(nonstd::function_traits<F>::arity == arg_list.size() && "subscriber and invoker differ in arguments count");
		return invoke(deduction_helper, ((Arg_Idx < arg_list.size()) ? arg_list[Arg_Idx] : create_arg<nonstd::param_types_t<F, Arg_Idx>>()) ...);
	}
	...
	{
		const auto all_params_are_convertible = nonstd::check_all_true(check_arg_type<nonstd::param_types_t<F, ArgCount>>(args)...);
...
public:
	...
	void disconnect(const id_t& id, C * const object_ptr, F&& f)
	{
		...
		using arg_index_sequence = nonstd::make_index_sequence< nonstd::function_traits<F>::arity >;
		auto slot = typename function_wrapper<F, arg_index_sequence>::delegate_t(object_ptr, std::forward<F>(f));
...
public:
	...
	R invoke(const id_t& id, Args&&... args) const
	{
		...
		assert((it != _list.end()) && "invoking a function without being binded to any and expecting a result");
		...
		{
			_list.erase(it);
			assert(false && "invoking a function without being binded to any and expecting a result");
		}
