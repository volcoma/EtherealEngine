...
// an memory pool is a specialized allocator that allocates memory in large
// chunks
// and deals them out in small slice, with O(1) amortized allocation and
// deallocation.
// its useful for a variety of cases:
// 1. reducing system call overhead when requesting multiple small memory
// blocks;
