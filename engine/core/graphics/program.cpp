...
void program::set_texture(std::uint8_t _stage,
                          const std::string& _sampler,
                          gfx::frame_buffer* frameBuffer,
                          ...
                          std::uint32_t _flags /*= std::numeric_limits<std::uint32_t>::max()*/)
{
    ...
    gfx::set_texture(_stage,
                     get_uniform(_sampler, true)->native_handle(),
                     frameBuffer->get_texture(_attachment)->native_handle(),
                     _flags);
}
