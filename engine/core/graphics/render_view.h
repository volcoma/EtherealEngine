...
public:
	std::shared_ptr<texture> get_texture(const std::string& id, std::uint16_t _width, std::uint16_t _height,
										 bool _hasMips, std::uint16_t _numLayers, texture_format _format,
										 std::uint32_t _flags = get_default_rt_sampler_flags(),
										 const memory_view* _mem = nullptr);

	std::shared_ptr<texture> get_texture(const std::string& id, backbuffer_ratio _ratio, bool _hasMips,
										 std::uint16_t _numLayers, texture_format _format,
										 std::uint32_t _flags = get_default_rt_sampler_flags());

	std::shared_ptr<texture> get_texture(const std::string& id, std::uint16_t _width, std::uint16_t _height,
										 std::uint16_t _depth, bool _hasMips, texture_format _format,
										 std::uint32_t _flags = get_default_rt_sampler_flags(),
										 const memory_view* _mem = nullptr);
