...
struct ImGuiDocker::ImGuiDockerImpl
{
	enum EndAction_
	{
		EndAction_None,
	...
	bool dockSlots(Dock& dock, Dock* dest_dock, const ImRect& rect, bool on_border)
	{
		...
		bool hasAnyDocked = getDocksWithStatus(Status_Docked) > 0;
		if (!hasAnyDocked)
			on_border = false;

		int maxSlots = (on_border && hasAnyDocked ? 4 : 5);
		int startSlot = hasAnyDocked ? 0 : maxSlots - 1;
    ...
    void debugWindow() {
        ...
        if (Begin("Dock Debug Info")) {
            for (int i = 0; i < m_docks.size(); ++i) {
                if (TreeNode(m_docks[i], "Dock %d (%p)", i, m_docks[i])) {
                    Dock &dock = *m_docks[i];
	...
	Dock* getDockByIndex(int idx) { return idx < 0 ? 0 : m_docks[(int)idx]; }

	int getDocksWithStatus(Status_ status)
	{
		int count = 0;
		for (int i = 0; i < m_docks.size(); ++i)
		{
			if (status == m_docks[i]->status)
				count++;
		}
...
ImGuiDocker::ImGuiDocker()
	: pImpl(new ImGuiDockerImpl())
{
...
{
	ImGuiContext& g = *GImGui;
    g.Docker.pImpl->m_next_dock_slot = slot;
}
...
void ImGui::BeginDockspace() 
{
	...
	ImGuiContext& g = *GImGui;
// 	ImVec4 color = g.Style.Colors[ImGuiCol_WindowBg];
// 	if (g.Docker.pImpl->getDocksWithStatus(ImGuiDocker::ImGuiDockerImpl::Status_Docked) < 1)
// 	{
// 		color.w = 0;
// 	}
// 
...
// 	PushStyleColor(ImGuiCol_WindowBg, color);
	PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(4.0f, g.Style.WindowPadding.y));

    Begin("###workspace", 0, flags);
	PopStyleVar(1);
	//PopStyleColor();
	
	g.Docker.pImpl->m_workspace_pos = GetWindowPos();
	g.Docker.pImpl->m_workspace_size = GetWindowSize();
}
...
{
	ImGuiContext& g = *GImGui;
	return g.Docker.pImpl->begin(label, opened, extra_flags);
}
...
{
	ImGuiContext& g = *GImGui;
	g.Docker.pImpl->debugWindow();
}

#include "Core/config/cfg.h"

void ImGui::SaveDock(ImGuiContext* context, const char* file)
{

	cfg::Config config;
	{
		ImGuiContext& g = *context;
		config.set_value("Global", "Count", g.Docker.pImpl->m_docks.size());
		for (int i = 0; i < g.Docker.pImpl->m_docks.size(); ++i)
		{
			auto& dock = *g.Docker.pImpl->m_docks[i];
			std::string strEntry = std::string("Dock") + std::to_string(i);
			config.set_value(strEntry, "Label", std::string(dock.label));
			config.set_value(strEntry, "X", dock.pos.x);
			config.set_value(strEntry, "Y", dock.pos.y);
			config.set_value(strEntry, "SizeX", dock.size.x);
			config.set_value(strEntry, "SizeY", dock.size.y);
			config.set_value(strEntry, "Status", (int)dock.status);
			config.set_value(strEntry, "Opened", dock.opened);
			config.set_value(strEntry, "Active", dock.active);
			config.set_value(strEntry, "Prev", (int)g.Docker.pImpl->getDockIndex(dock.prev_tab));
			config.set_value(strEntry, "Next", (int)g.Docker.pImpl->getDockIndex(dock.next_tab));
			config.set_value(strEntry, "Child0", (int)g.Docker.pImpl->getDockIndex(dock.children[0]));
			config.set_value(strEntry, "Child1", (int)g.Docker.pImpl->getDockIndex(dock.children[1]));
			config.set_value(strEntry, "Parent", (int)g.Docker.pImpl->getDockIndex(dock.parent));
...
void ImGui::LoadDock(ImGuiContext* context, const char* file)
{
	ImGuiContext& g = *context;

	for (int i = 0; i < g.Docker.pImpl->m_docks.size(); ++i)
	{
		g.Docker.pImpl->m_docks[i]->~Dock();
		MemFree(g.Docker.pImpl->m_docks[i]);
	}
	g.Docker.pImpl->m_docks.clear();

	cfg::Config config;
	config.load(file);
	{
		std::size_t nCount = config.get_value<std::size_t>("Global", "Count", 0);

		for (std::size_t i = 0; i < nCount; ++i)
		{
			ImGuiDocker::ImGuiDockerImpl::Dock* new_dock = (ImGuiDocker::ImGuiDockerImpl::Dock*)MemAlloc(sizeof(ImGuiDocker::ImGuiDockerImpl::Dock));
			g.Docker.pImpl->m_docks.push_back(IM_PLACEMENT_NEW(new_dock) ImGuiDocker::ImGuiDockerImpl::Dock());
		}
		...
		for (int i = 0; i < (int)nCount; ++i)
		{
			...
			std::string label = config.get_value<std::string>(strEntry, "Label", "");
			dock.label = ImStrdup(label.c_str());
			dock.id = ImHash(dock.label, 0);
			dock.pos.x = config.get_value<float>(strEntry, "X", 0);
			dock.pos.y = config.get_value<float>(strEntry, "Y", 0);
			dock.size.x = config.get_value<float>(strEntry, "SizeX", 0);
			dock.size.y = config.get_value<float>(strEntry, "SizeY", 0);
			dock.status = (ImGuiDocker::ImGuiDockerImpl::Status_)config.get_value<int>(strEntry, "Status", 0);
			dock.opened = config.get_value<bool>(strEntry, "Opened", false);
			dock.active = config.get_value<bool>(strEntry, "Active", true);
			dock.prev_tab = g.Docker.pImpl->getDockByIndex(config.get_value<int>(strEntry, "Prev", -1));
			dock.next_tab = g.Docker.pImpl->getDockByIndex(config.get_value<int>(strEntry, "Next", -1));
			dock.children[0] = g.Docker.pImpl->getDockByIndex(config.get_value<int>(strEntry, "Child0", -1));
			dock.children[1] = g.Docker.pImpl->getDockByIndex(config.get_value<int>(strEntry, "Child1", -1));
			dock.parent = g.Docker.pImpl->getDockByIndex(config.get_value<int>(strEntry, "Parent", -1));
