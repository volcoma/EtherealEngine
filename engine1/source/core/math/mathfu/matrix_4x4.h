...
class Matrix<float, 4> {
 public:
  Matrix<float, 4>() {
	  simd4x4f_identity(&data_.simd_matrix);
  }
  ...
  inline bool operator == (const Matrix<float, 4> & mat) const
  {
	  return (memcmp(this->data_.float_array, &mat.data_.float_array, sizeof(this->data_.float_array)) == 0);
  }
  ...
  inline Vector<float, 3> AxisX3D() const {
	  Vector<float, 3> return_v;
	  MATHFU_VECTOR3_STORE3(FindColumn(0).simd, return_v.data_);
	  return return_v;
  }

  inline Vector<float, 3> AxisY3D() const {
	  Vector<float, 3> return_v;
	  MATHFU_VECTOR3_STORE3(FindColumn(1).simd, return_v.data_);
	  return return_v;
  }

  inline Vector<float, 3> AxisZ3D() const {
	  Vector<float, 3> return_v;
	  MATHFU_VECTOR3_STORE3(FindColumn(2).simd, return_v.data_);
	  return return_v;
  }
  ...
  static inline const Matrix<float, 4>& Identity() {
    static Matrix<float, 4> return_m;
