...
vec4 pbr_light(vec2 texcoord0)
{
	...
	vec3 world_position = clipToWorld(u_mtx, clip);
	...
	vec3 energy = AreaLightSpecular(0.0f, 5.0f, normalize(vector_to_light), lobe_roughness, vector_to_light, L, V, N);
	vec3 surface_lighting = StandardShading(albedo_color, specular_color, lobe_roughness, energy, L, V, N);
