...
{
	_obb.m_mtx[0][0] = (_aabb.m_max[0] - _aabb.m_min[0]) * 0.5f;
	_obb.m_mtx[1][1] = (_aabb.m_max[1] - _aabb.m_min[1]) * 0.5f;
	_obb.m_mtx[2][2] = (_aabb.m_max[2] - _aabb.m_min[2]) * 0.5f;
	_obb.m_mtx[3][0] = (_aabb.m_min[0] + _aabb.m_max[0]) * 0.5f;
	_obb.m_mtx[3][1] = (_aabb.m_min[1] + _aabb.m_max[1]) * 0.5f;
	_obb.m_mtx[3][2] = (_aabb.m_min[2] + _aabb.m_max[2]) * 0.5f;
	_obb.m_mtx[3][3] = 1.0f;
}
...
void sphereToAabb(Aabb& _aabb, const Sphere& _sphere)
{
	...
	_aabb.m_min = _sphere.m_center - radius;
	_aabb.m_max = _sphere.m_center + radius;

}

void aabbTransformToObb(Obb& _obb, const Aabb& _aabb, const math::mat4 _mtx)
{
	aabbToObb(_obb, _aabb);
	math::mat4 result = _obb.m_mtx * _mtx;
	_obb.m_mtx = result;
}
...
void calcAabb(Aabb& _aabb, const math::mat4& _mtx, const void* _vertices, uint32_t _numVertices, uint32_t _stride)
{
	...
	math::vec3 position;
	math::vec4 vertexPos = math::vec4(*(math::vec3*)vertex, 0);
	position = _mtx * vertexPos;
	min[0] = max[0] = position[0];
	min[1] = max[1] = position[1];
	min[2] = max[2] = position[2];
	vertex += _stride;
...
void calcObb(Obb& _obb, const void* _vertices, uint32_t _numVertices, uint32_t _stride, uint32_t _steps)
{
	...
	for (uint32_t ii = 0; ii < _steps; ++ii)
	{
		...
		for (uint32_t jj = 0; jj < _steps; ++jj)
		{
			...
			{
				mtx = math::rotate(mtx, ax, math::vec3{ 1.0f, 0.0f, 0.0f });
				math::mat4 mtxT = math::transpose(mtx);
...
void calcMaxBoundingSphere(Sphere& _sphere, const void* _vertices, uint32_t _numVertices, uint32_t _stride)
{
	...
	for (uint32_t ii = 0; ii < _numVertices; ++ii)
	{
		...
		maxDistSq = math::max(distSq, maxDistSq);
	}
	_sphere.m_center = center;
	_sphere.m_radius = math::sqrt(maxDistSq);
}
	...
	do
	{
		...
		for (uint32_t ii = 0, index = math::linearRand<std::uint32_t>(0, _numVertices); ii < _numVertices; ++ii, index = (index + 1)%_numVertices)
		{
			...
			if (distSq > maxDistSq)
			{
				...
				maxDistSq = math::lerp(maxDistSq, distSq, _step);
...
void buildFrustumPlanes(Plane* _result, const float* _viewProj)
{
	...
	{
		float len = math::length(plane->m_normal);
		plane->m_normal = math::normalize(plane->m_normal);
...
{
	math::vec3 axb = math::cross(_pa.m_normal, _pb.m_normal);

	math::vec3 bxc = math::cross(_pb.m_normal, _pc.m_normal);

	math::vec3 cxa = math::cross(_pc.m_normal, _pa.m_normal);

	math::vec3 tmp0 = bxc * _pa.m_dist;

	math::vec3 tmp1 = cxa * _pb.m_dist;

	math::vec3 tmp2 = axb * _pc.m_dist;

	math::vec3 tmp = tmp0 + tmp1;
	tmp0 = tmp + tmp2;

	float denom = math::dot(_pa.m_normal, bxc);
	_result = tmp0 * (-1.0f / denom);
}
// 
