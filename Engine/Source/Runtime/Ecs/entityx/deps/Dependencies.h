...
namespace entityx
{
	namespace deps 
	{
		...
		{
		public:
			void receive(const ComponentAddedEvent<Component> &event)
			{
				if(event.component.lock()->getId_v() == C::getId())
					assign<Deps...>(event.entity);
			}

			void configure(EventManager &events) override 
			{
				events.subscribe<ComponentAddedEvent<Component>>(*this);
			}
