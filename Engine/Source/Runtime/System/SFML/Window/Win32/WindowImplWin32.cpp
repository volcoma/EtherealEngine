...
namespace
{
    ...
    const char*             className        = "SFML_Window";
...
////////////////////////////////////////////////////////////
WindowImplWin32::WindowImplWin32(VideoMode mode, const std::string& title, std::uint32_t style) :
m_handle          (NULL),
m_callback        (0),
m_cursor          (NULL),
m_loadedCursor    (NULL),
...
m_cursorGrabbed   (m_fullscreen)
{
    ...
    // Create the window
    m_handle = CreateWindow(className, title.c_str(), win32Style, left, top, width, height, NULL, NULL, GetModuleHandle(NULL), this);
...
////////////////////////////////////////////////////////////
void WindowImplWin32::setIcon(unsigned int width, unsigned int height, const std::uint8_t* pixels)
{
	if (width == 0 || height == 0)
		return;
    ...
    switch (message)
    {
        ...
        case WM_SIZE:
        {
            ...
            if (wParam != SIZE_MINIMIZED && !m_resizing && m_lastSize != getSize())
            {
                ...
                event.size.width  = m_lastSize.width;
                event.size.height = m_lastSize.height;
                pushEvent(event);
