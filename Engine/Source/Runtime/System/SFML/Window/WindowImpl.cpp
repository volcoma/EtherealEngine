...
{
////////////////////////////////////////////////////////////
WindowImpl* WindowImpl::create(VideoMode mode, const std::string& title, std::uint32_t style)
{
...
m_joystickThreshold(0.1f)
{
    ...
    for (unsigned int i = 0; i < Sensor::Count; ++i)
		m_sensorValue[i] = std::array<float, 3>{0.0f, 0.0f, 0.0f};
}
...
void WindowImpl::processSensorEvents()
{
    ...
    for (unsigned int i = 0; i < Sensor::Count; ++i)
    {
        ...
        if (SensorManager::getInstance().isEnabled(sensor))
        {
			...
			std::array<float, 3> previousValue = m_sensorValue[i];
            ...
            if (m_sensorValue[i] != previousValue) // @todo use a threshold?
            {
                ...
                event.sensor.x = m_sensorValue[i][0];
                event.sensor.y = m_sensorValue[i][1];
                event.sensor.z = m_sensorValue[i][2];
                pushEvent(event);
            }
        }
    }
