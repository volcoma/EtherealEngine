...
// 		Orientation.w = w;
		int i, j, k = 0;
		float fRoot, fTrace = Row[0].x + Row[1].y + Row[2].z;
		if (fTrace > 0.0f)
		{
			fRoot = sqrt(fTrace + 1.0f);
			Orientation.w = 0.5f * fRoot;
			fRoot = 0.5f / fRoot;
			Orientation.x = fRoot * (Row[1].z - Row[2].y);
			Orientation.y = fRoot * (Row[2].x - Row[0].z);
			Orientation.z = fRoot * (Row[0].y - Row[1].x);
		...
		else
		{
			...
			fRoot = sqrt(Row[i][i] - Row[j][j] - Row[k][k] + 1.0f);

			Orientation[i] = 0.5f * fRoot;
			fRoot = 0.5f / fRoot;
			Orientation[j] = fRoot * (Row[i][j] + Row[j][i]);
			Orientation[k] = fRoot * (Row[i][k] + Row[k][i]);
			Orientation.w = fRoot * (Row[j][k] - Row[k][j]);
