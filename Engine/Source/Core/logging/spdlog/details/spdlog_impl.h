...
inline std::shared_ptr<spdlog::logger> spdlog::get_shared(const std::string& name)
{
    return details::registry::instance().get(name);
}

inline spdlog::logger* const spdlog::get(const std::string& name)
{
	return details::registry::instance().get(name).get();
}
