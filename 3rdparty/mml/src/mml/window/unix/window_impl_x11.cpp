...
////////////////////////////////////////////////////////////
window_impl_x11::window_impl_x11(video_mode mode, const std::string& title, unsigned long style, const ContextSettings& settings) :
_window         (0),
_screen         (0),
_input_method    (NULL),
_input_context   (NULL),
...
_last_input_time  (0)
{
    ...
    XVisualInfo visualInfo = ContextType::selectBestVisual(_display, mode.bits_per_pixel, settings);
    ...
    if (_fullscreen)
    {
        set_position(std::array<std::int32_t, 2>(0, 0));
        set_video_mode(mode);
...
std::array<std::uint32_t, 2> window_impl_x11::get_size() const
{
    ...
    return std::array<std::uint32_t, 2>(attributes.width, attributes.height);
}
...
void window_impl_x11::set_size(const std::array<std::uint32_t, 2>& size)
{
    ...
    XResizeWindow(_display, _window, size.x, size.y);
    XFlush(_display);
}
...
{
	X11_XIconifyWindow(_display, _window, _screen);
...
void window_impl_x11::set_alpha(float alpha)
{
	...
	unsigned long opacity = (0xffffffff / 0xff) * unsigned char(alpha * 255);
...
bool window_impl_x11::process_event(XEvent& windowEvent)
{
    ...
    switch (windowEvent.type)
    {
        ...
        case ConfigureNotify:
        {
            ...
            if ((windowEvent.xconfigure.width != _previous_size.x) || (windowEvent.xconfigure.height != _previous_size.y))
            {
                platform_event event;
                ...
                _previous_size.x = windowEvent.xconfigure.width;
                _previous_size.y = windowEvent.xconfigure.height;
            }
            break;
        }
                ...
                if (_input_context)
                {
                    ...
                    if (length > 0)
                    {
                        ...
                        if (unicode != 0)
                        {
                            ...
                            textEvent.type         = platform_event::TextEntered;
