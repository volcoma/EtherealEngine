...
#undef HEX_DUMP_FORMAT
	...
	int compileShader(int _argc, const char* _argv[], bx::MemoryBlock& memBlock, int64_t& sz, std::string& err)
	{
		...
		if (cmdLine.hasArg('v', "version") )
		{
			...
			bx::stringPrintf(err, "shaderc, bgfx shader compiler tool, version %d.%d.%d.\n"
				, BGFX_SHADERC_VERSION_MAJOR
				, BGFX_SHADERC_VERSION_MINOR
				, BGFX_API_VERSION
			);
...
// 							}
							...
							||  0 != essl)
							{
								...
								compiled = true;
	#else
								compiled = compileGLSLShader(cmdLine, essl, code, writer, err);
	#endif // 0
							}
							else if (0 != spirv)
							{
						...
						if ('f' == shaderType)
						{
							...
							if (hasPrimitiveId)
							{
								...
								else
								{
									...
									bx::stringPrintf(err, "gl_PrimitiveID builtin is not supported by this D3D9 HLSL.\n");
									return EXIT_FAILURE;
								}
							}
						...
						else if ('v' == shaderType)
						{
							...
							if (hasVertexId)
							{
								...
								else
								{
									...
									bx::stringPrintf(err, "gl_VertexID builtin is not supported by this D3D9 HLSL.\n");
									return EXIT_FAILURE;
								}
							}
							...
							if (hasInstanceId)
							{
								...
								else
								{
									...
									bx::stringPrintf(err, "gl_InstanceID builtin is not supported by this D3D9 HLSL.\n");
									return EXIT_FAILURE;
								}
							}
