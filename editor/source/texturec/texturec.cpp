...
namespace bgfx
{
	const Memory* mem_alloc(uint32_t _size)
	{
	...
	const Memory* mem_makeRef(const void* _data, uint32_t _size)
	{
	...
	void mem_release(const Memory* _mem)
	{
...
int compile_texture(int _argc, const char* _argv[])
{
...

	{
		...
		{
			mem_release(mem);
			mem = mem_makeRef(imageContainer.m_data, imageContainer.m_size);
		}
